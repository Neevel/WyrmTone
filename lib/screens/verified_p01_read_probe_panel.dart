...
const matriboxP01ReadProbeEnabled =
    kDebugMode &&
    bool.fromEnvironment('ENABLE_MATRIBOX_P01_READ_PROBE', defaultValue: false);

/// Separate, single-purpose channel client for the experimental one-shot
/// P01 read probe. Deliberately NOT merged into [VerifiedMatriboxProbePanel]
/// (the write probes): it never accepts a bank, a slot or a byte array, and
/// must not become a generic SysEx sender by association. Native Kotlin
/// owns the immutable fixed request; this widget only calls the
/// parameterless native methods and interprets the returned raw bytes with
/// the shared, reused offline decoder.
class VerifiedP01ReadProbePanel extends StatefulWidget {
  const VerifiedP01ReadProbePanel({
    required this.connectionReady,
    required this.monitoring,
    this.enabled = matriboxP01ReadProbeEnabled,
    super.key,
  });
  ...
  @override
  State<VerifiedP01ReadProbePanel> createState() => _P01ReadProbePanelState();
}

class _P01ReadProbePanelState extends State<VerifiedP01ReadProbePanel>
  ...
  @override
  void didUpdateWidget(VerifiedP01ReadProbePanel oldWidget) {
    ...
    try {
      final status =
          await _channel.invokeMapMethod<Object?, Object?>(
            'getVerifiedPresetP01ReadProbeStatus',
          ) ??
          {};
    ...
    try {
      ...
      final accepted = await showDialog<bool>(
        ...
        builder: (context) => AlertDialog(
          title: const Text('Jetzt einmalig den P01-Read-Request senden?'),
          content: const Text(
            'Sonicake Matribox 1 · experimenteller Read-Test\n'
            'Es wird exakt eine fest einprogrammierte, 17 Byte lange '
            'Anfrage gesendet (der bestätigte Connect-Sync-Trigger für '
            'User-Bank Slot 0 / P01). Kein Speichern, kein Auswählen, '
            'kein Parameterschreiben, keine Bank-/Slotwahl.\n'
            'Antwortet das Gerät nicht innerhalb weniger Sekunden, gilt '
            'der Test als abgelaufen. Keine automatische Wiederholung.',
          ),
          actions: [
            TextButton(
            ...
            FilledButton(
              key: const Key('p01-read-probe-confirm'),
      ...
      // Intentionally parameterless. Kotlin owns the immutable fixed request.
      final result =
          await _channel.invokeMapMethod<Object?, Object?>(
      ...
      final rawChunks = (result['chunks'] as List<Object?>? ?? [])
          ...
          .map((chunk) => (chunk['bytes'] as List<Object?>? ?? [])
              .whereType<int>()
              .toList())
          .toList();
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      ...
      child: ExpansionTile(
        key: const Key('verified-p01-read-probe-panel'),
        initiallyExpanded: false,
        title: const Text('Experimenteller P01-Read-Einmaltest'),
        ...
        children: [
          const Text(
            ...
            'Sendet genau eine fest einprogrammierte 17-Byte-Anfrage und '
            'wertet ausschließlich die Geräteantwort aus.\n'
            'Kein Speichern, kein Auswählen, kein Parameterschreiben, keine '
            'Bank-/Slotwahl.\n'
            'Rein lesend: bestätigt oder widerlegt, ob dieser Request '
            'isoliert einen P01-Readback auslöst.',
          ),
          ...
          CheckboxListTile(
            key: const Key('p01-read-probe-checkbox'),
            value: _checked,
            onChanged: available
