  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        ...
        return Scaffold(
          appBar: AppBar(title: const Text('Empfehlung')),
          body: ListView(
              ...
              if (widget.controller.profiles.isNotEmpty)
                DropdownButtonFormField<String>(
                  ...
                  items: widget.controller.profiles
                      ...
                      .toList(),
                  onChanged: (value) {
                    if (value != null) widget.controller.selectProfile(value);
                  },
                ),
              const SizedBox(height: 8),
              DropdownButtonFormField<String>(
                initialValue: sound.id,
                decoration: const InputDecoration(labelText: 'Ziel-Sound'),
              ...
              if (widget.controller.selectedTargetDevice ==
                ...
                if (widget.controller.selectedTargetDevice ==
                  ...
                  if (widget.tone3000 == null ||
                      widget.controller
                          .namRecommendations(widget.tone3000!.namCaptures)
                          .isEmpty)
                    const Text(
                ...
                if (!recommendation.cabSimulationEnabled)
                  ...
                  Column(
                    ...
                    children: [
                      ...
                      if (widget.tone3000 case final tone3000?)
                        TextButton.icon(
                          key: const Key('recommendation-tone3000-browse'),
                  ...
                  for (final candidate in recommendation.irCandidates)
                    Card(
                      key: Key('ir-recommendation-${candidate.ir.fileName}'),
                      child: Column(
                        children: [
                          ListTile(
                  ...
                  children: RecommendationFeedback.values
                      .map(
                        (item) => ChoiceChip(
                          ...
                          onSelected: (_) => setState(() => feedback = item),
                        ),
                      )
                      .toList(),
                ),
        ...
        for (final candidate in recommendations)
          Card(
            child: ListTile(
              ...
              trailing: Text('${candidate.score} %'),
              subtitle: Text(
                'Kompatibilität: ${candidate.capture.compatibility.name}\n'
                '${candidate.reasons.join(' ')}\n'
                'Zusätzliche IR: ${candidate.additionalIrRequired == null
                    ? 'unklar – Warnung'
                    : candidate.additionalIrRequired!
